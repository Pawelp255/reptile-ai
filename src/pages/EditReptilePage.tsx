...
export default function EditReptilePage() {
  ...
  useEffect(() => {
    const loadReptile = async () => {
      ...
      try {
        const reptile = await getReptileById(id);
        if (!reptile) {
          navigate('/reptiles');
          return;
        }
        ...
        setFormData({
          ...
          animalGroup: resolveAnimalGroup(reptile),
          animalClass: reptile.animalClass || 'reptile',
          animalCategory: reptile.animalCategory || 'other-reptile',
          speciesGroup: reptile.speciesGroup || '',
          morph: reptile.morph || '',
          sex: reptile.sex,
          ...
          breedingStatus: reptile.breedingStatus || 'pet',
          notes: reptile.notes || '',
          ...
          isVenomous: reptile.isVenomous || false,
          isDangerous: reptile.isDangerous || false,
          isAmphibian: reptile.isAmphibian || reptile.animalClass === 'amphibian',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id || !formData.name.trim() || !formData.species.trim()) {
      return;
    }
    ...
    try {
      ...
      await updateReptile(id, {
        ...
        animalClass: formData.animalClass,
        ...
        uvbRequirement: formData.uvbRequirement,
        waterRequirement: formData.waterRequirement,
        handlingProfile: formData.handlingProfile,
        isVenomous: formData.isVenomous || undefined,
        isDangerous: formData.isDangerous || undefined,
        isAmphibian: formData.isAmphibian || undefined,
      ...
      toast.success('Changes saved locally', {
        description: 'Animal profile updated on this device.',
      });
      ...
      <form id="edit-reptile-form" onSubmit={handleSubmit} className="p-4 space-y-6 pb-32">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Note: Changing diet type won't update existing schedule frequencies.
            </p>
          </div>
        </div>
